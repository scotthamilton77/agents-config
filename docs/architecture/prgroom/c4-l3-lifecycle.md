...
| `escalate_if_needed` | Cross-cutting hook that fires one `EscalationSink` event per item per cycle when the item's `Disposition.Kind ∈ {escalated, failed}` AND `EscalationFiled == false`. Per §3.2 priority 3 rules. |
| `request_human_review_if_needed` | Cross-cutting hook (§4.7) that POSTs the `human-review-required` label to the PR via the gh adapter when `Phase=human-gated` AND `state.HumanReviewLabelAdded == false`; sets the flag to true. |
| Contract A | Cluster-bundling agent dispatch (§5). Cheap; local-first chain ollama → claude haiku → codex-mini. |
| Contract B | Per-cluster fix agent dispatch (§5). `sonnet[1m]` orchestrator that decides per-comment disposition AND implements. |
...
C4Component
    ...
    Container_Boundary(binary, "prgroom binary") {
        ...
        Container_Boundary(lifecycle, "internal/lifecycle") {
            ...
            Component(cluster, "clusterLocked", "Go function", "Dispatches Contract A to bundle unprocessed items into fix-clusters. Sets ClusterID on items. Idempotent on already-clustered items.")
            Component(fix, "fixLocked", "Go function", "Per cluster: dispatches Contract B; receives per-item Disposition + commit SHAs; runs §5 contract-audit (commit-orphan check, schema validation, commit-shas-on-branch check); flips affected items to Disposition.Kind=failed on audit failure.")
            ...
            Component(resolver, "resolve_end_of_cycle_phase", "Go function", "§3.2 priority cascade. Six conditions evaluated in strict priority order: hard-cap > failed items > escalated items > push-happened > quiescence-trips > stay-in-fixes-pending. First match wins.")
            ...
            Component(escalate_hook, "escalate_if_needed", "Go function", "Cross-cutting hook called by runLocked after each *Locked. Emits one EscalationSink event per item with Disposition.Kind ∈ {escalated, failed} AND EscalationFiled == false. Sets the per-item EscalationFiled flag.")
            Component(humanreq_hook, "request_human_review_if_needed", "Go function", "§4.7 cross-cutting hook called by runLocked when end-of-cycle resolves to human-gated AND state.HumanReviewLabelAdded == false. POSTs the human-review-required label to GitHub via the gh adapter; sets HumanReviewLabelAdded=true.")
        }
        ...
        Container_Boundary(agent_pkg, "internal/agent") {
            Component(contract_a, "ContractA — cluster", "Go function", "Cluster dispatch. Local-first chain ollama+Gemma → claude haiku → codex-mini.")
            Component(contract_b, "ContractB — fix", "Go function", "Fix dispatch. sonnet[1m] orchestrator. Returns per-item disposition + commit SHAs.")
        }
    ...
    Rel(run, escalate_hook, "After each *Locked")
    Rel(run, humanreq_hook, "After end-of-cycle if Phase=human-gated AND label not yet added")
...
function runLocked(pr, mode) (*PRGroomingState, error):
    ...
    for {
        # Loop-top terminal check — exits cleanly for any terminal-for-CLI phase
        if state.Phase in {quiesced, human-gated, merged}:
            return state, nil

        # The cycle: each *Locked runs under handle_verb_error
        state, err = pollLocked(pr, state);            if r := handle_verb_error(err); r.Propagate { return state, err }
        state, err = clusterLocked(pr, state);         if r := handle_verb_error(err); r.Propagate { return state, err }
        state, err = fixLocked(pr, state);             if r := handle_verb_error(err); r.Propagate { return state, err }
        state, err = pushLocked(pr, state);            if r := handle_verb_error(err); r.Propagate { return state, err }
        if push_uploaded_commits_this_cycle(state):
            state, err = rereviewLocked(pr, state);    if r := handle_verb_error(err); r.Propagate { return state, err }
        state, err = replyLocked(pr, state);           if r := handle_verb_error(err); r.Propagate { return state, err }
        state, err = resolveLocked(pr, state);         if r := handle_verb_error(err); r.Propagate { return state, err }

        # Cross-cutting hooks AFTER all *Locked
        escalate_if_needed(state)                       # one Escalation per qualifying item per cycle
        ...
        if state.Phase == human-gated AND !state.HumanReviewLabelAdded:
            request_human_review_if_needed(state)       # POST label, set HumanReviewLabelAdded=true

        # Wait if the resolver landed in awaiting-review / idle
        if state.Phase in {awaiting-review, idle}:
            state, err = waitLocked(pr, state);         if r := handle_verb_error(err); r.Propagate { return state, err }

        # Loop back to terminal check
    }
```
...
The dependencies between them are linear (the order in `runLocked`'s for-loop) — there is no fan-out, no parallel verb dispatch in MVP. **Cluster + fix do fan out across clusters within a single verb invocation** (each cluster is one Contract A or Contract B subprocess), but the per-verb loops over clusters serialise.

### Cross-cutting components

- **`escalate_if_needed`** fires AFTER all `*Locked` calls have completed for a cycle. It iterates `state.Items` once and emits a single `Escalation` per item whose `Disposition.Kind ∈ {escalated, failed}` AND `EscalationFiled == false`, then sets the per-item flag. This dedupes — an item that was already escalated in a prior cycle does not re-fire.
- **`request_human_review_if_needed`** fires AFTER `resolve_end_of_cycle_phase` has written `Phase=human-gated` AND `state.HumanReviewLabelAdded` is still false. It POSTs the `human-review-required` label via the gh adapter and sets the flag. The flag is reset on the next successful end-of-cycle resolution that writes a non-`human-gated` phase, so subsequent gates re-add the label.
...
| `internal/agent` | `ContractA` + `ContractB` | Cluster / fix subprocess dispatch |
...
- **Contract A / Contract B internals.** This diagram surfaces them as components inside `internal/agent`; the per-contract provider chains, prompt templates, token-usage JSONL emitter, and audit-rule mechanics live in [`c4-l3-agent-dispatch.md`](c4-l3-agent-dispatch.md) (stub).
