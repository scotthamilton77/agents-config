...
- **Token-usage JSONL emitter** — appends one line per agent invocation to `$XDG_STATE_HOME/prgroom/token-usage.jsonl`. Schema per §5: `{ts, pr, contract, provider, model, input_tokens, output_tokens, duration_ms, outcome}`. MVP does no aggregation — the file is the baseline data; analysis is a v2 deferral.
