...
- **Installer owns the install.** `scripts/install.sh` is a thin stub that execs the Python installer (`packages/installer`); its CLI-deploy stage (`core/clis.py` + `core/run.py`'s `deploy_clis`/`prune_clis`) runs `uv tool install` against `packages/prgroom/` on every user-path run — idempotent, digest-gated (a source change prompts a consent-gated upgrade; docs/test churn does not), `--force` on an accepted takeover — guarded by the `MIN_UV_VERSION` check; `--prune`/`--prune-only` removes a retired entry via `uv tool uninstall`. The executable that the operator / scheduler / wrapping skill `exec`s is the one this install path put on `PATH`, receipt-tracked in `install-receipt.json`'s `clis` field.
- **Versioned with the repo, not released independently.** The version lives in `packages/prgroom/pyproject.toml` (semver). There is **no independent release cadence, no git tags, and no GitHub releases** for the MVP — `prgroom` ships with the repo, and upgrades land by re-running the installer with `--yes` or an interactive accept (its CLI-deploy stage detects a changed `cli_source_digest` and offers the upgrade; a full reinstall still works via `uv tool install --force`). Same lifecycle as `installer` and `pdlc`.
