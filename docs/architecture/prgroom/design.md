...
The registered command surface is this set plus two verbs that are not part of it. `approve` submits the GitHub-App-attested approving review; `post-verdict` submits the App's comment-only review carrying a review round's verdict, and never approves — the two reviews stay separate so a verdict cannot double as merge authorization. Neither takes a PR lock, neither reads or writes grooming state, and `run` threads neither. Both have rows in the "Verb → contract → CLI action" table (§5).
