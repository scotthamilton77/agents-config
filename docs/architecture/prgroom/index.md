...
| Verb | A prgroom subcommand. The MVP verb set is `poll`, `cluster`, `fix`, `verify`, `push`, `rereview`, `reply`, `resolve`, `resolve-escalated`, `wait`, `status`, `run`, `sweep`. |
...
| Round | A single review-iteration loop: poll → cluster → fix → verify → push → re-review → reply → resolve. The outer bound is the `pr_review_retries` retry budget (the PR-review retry cap): an exhausted budget escalates to `human-gated` with `LIFECYCLE_PR_REVIEW_EXHAUSTED`. |
...
- prgroom's runtime behaviour: one PR-grooming session's traversal through poll → cluster → fix → verify → push → re-review → reply → resolve → wait, including the four canonical flows (happy, bot-silence, PR-review-retry exhaustion, resumability)
