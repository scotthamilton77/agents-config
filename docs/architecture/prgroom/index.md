...
| Verb | A prgroom subcommand. The MVP verb set is `poll`, `cluster`, `fix`, `push`, `rereview`, `reply`, `resolve`, `resolve-escalated`, `wait`, `status`, `run`, `sweep`. (`verify` and `cap-guard` are internal pre-push pipeline steps, **not** subcommands.) |
...
| Round | A single review-iteration loop. Pipeline steps in order: poll → cluster → fix → verify → cap-guard → push → reply → resolve → rereview (`rereview` runs last, guarded). The outer bound is the `pr_review_retries` retry budget (the PR-review retry cap): an exhausted budget escalates to `human-gated` with `LIFECYCLE_PR_REVIEW_EXHAUSTED`. |
| `verify` step | The mechanical gate of record. A pre-push `VerbStep` inserted between `fix` and `cap-guard` that runs the operator-configured tier command (whole-branch, via `proc.CommandRunner`) to confirm the branch is sound before a push elicits another review round. No-ops when there are no queued fix commits; on inner-budget exhaustion it flips `phase = HUMAN_GATED` (identical refusal mechanism to cap-guard). See [`c4-l3-verify.md`](c4-l3-verify.md). |
...
- prgroom's runtime behaviour: one PR-grooming session's traversal through poll → cluster → fix → verify → cap-guard → push → reply → resolve → rereview → wait, including the four canonical flows (happy, bot-silence, PR-review-retry exhaustion, resumability)
...
| [`c4-l3-lifecycle.md`](c4-l3-lifecycle.md) | drawn | **C4 Level 3** (lifecycle) — components inside `src/prgroom/lifecycle`: the `_run` control flow with verb breakdown (`_poll → _cluster → _fix → _verify → _push → _reply → _resolve → _rereview → _wait`), showing `escalate_if_needed` and `request_human_review_if_needed` call sites |
| [`c4-l3-verify.md`](c4-l3-verify.md) | drawn | **C4 Level 3** (fix↔verify subsystem) — the `verify` `VerbStep` inserted between `fix` and `cap-guard`: the mechanical tier gate (`GateStrength` whole-branch via `proc.CommandRunner`, the authoritative gate of record), the bounded convergence loop (red gate → temp-file gate output → whole-branch repair dispatch → re-audit → re-gate, bounded by `fix_verify_retries` → `LIFECYCLE_FIX_VERIFY_EXHAUSTED`), the trust-but-verify contract (armed fix agent's `verify_checklist` claim vs prgroom's authoritative confirmation), and the two retry caps (`fix_verify_retries` inner / `pr_review_retries` outer) |
