...
| `items[]` | projection over `state.items` | Per-item disposition surface (disposition-contract spec §3). One row per persisted `ReviewItem`: `kind`, `gh_id` (string — consumers must `tostring`-coerce live GitHub numeric ids before matching), `thread_id`, `author` (informational — never an exclusion key), `disposition` (`null` = untriaged, else exactly `{kind, decided_at, decided_by}`), `replied`, `resolved`, `posted_reply_ids` (strings — projected from the recorded reply ledger `own_reply_id`; the list shape keeps a future multi-reply storage change non-breaking; `[]` when no reply was recorded). `body_excerpt`, `rationale`, `commits`, `response_path`, `gate`, and cluster bookkeeping stay private to the store. |
