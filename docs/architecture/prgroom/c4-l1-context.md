...
C4Context
    ...
    System_Ext(gh_cli, "gh CLI (auth state)", "prgroom shells out to `gh`, reusing its existing auth token, scopes, and host config. prgroom does not store credentials.")
...
- **gh CLI (auth state)** — prgroom shells out to `gh`, reusing its existing auth token, scopes, and host config. It does NOT store credentials of its own. If the operator can `gh pr view <pr>`, prgroom can talk to GitHub.
