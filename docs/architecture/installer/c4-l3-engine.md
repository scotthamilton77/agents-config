...
C4Component
    ...
    Container_Boundary(proc, "installer process") {
        ...
        Container_Boundary(core, "core/ — pure, tool-agnostic engine") {
            ...
            Component(versions, "versions.py", "Python", "A CLI package's declared version: the release part, the +partial label, and the bump guard's decision. Read by run.py's CLI-deploy stage to refuse deploying a partial, and by version_guard_cli.py at the repo-side gate.")
...
- **The repo-side lint/gate CLIs** (`content_lint_cli.py`, `content_tests_cli.py`, `doc_lint_cli.py`, `spec_lint_cli.py`, `version_guard_cli.py`, and their `core/content_lint.py` / `core/content_tests.py` / `core/doc_lint.py` / `core/spec_lint.py` / `core/versions.py` engines) — each is its own entry point under `python -m installer.<name>_cli`, never called from `cli.py`, and never invokes the installer. They are out of scope for this diagram because it is scoped to "the installer process" (`python3 scripts/install.py` / `python -m installer`), which they are not part of; see the `Makefile`'s `content-lint`, `content-tests`, `doc-lint`, `spec-lint`, and `version-guard` targets for how they run. `core/versions.py` is the one engine the installer process also reaches: `run.py`'s CLI-deploy stage reads a package's declared version through it and refuses to deploy one carrying the partial label.
