...
- The installer's runtime behaviour: one install invocation's traversal through detect → stage → overlay → merge → admission gate (user-home path) → sync → (optional) prune, plus the collision-merge, per-item sync-decision, and prune sub-flows.
...
| [`sequences.md`](sequences.md) | **Four sequence diagrams**: (1) end-to-end install — detect → stage → overlay → merge → admission gate (user-home path) → sync; (2) collision-merge dispatch — `(FileKind, namespace)` → strategy; (3) sync per-item decision — hash-skip vs diff → confirm → backup → write; (4) prune flow — orphan scan → interactive prune → backup + remove. |
