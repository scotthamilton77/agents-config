...
| Installer | The Python package at `packages/installer/`, reached through `scripts/install.sh` — a thin `exec` stub that delegates to it. A short-lived CLI: parse argv → stage in memory → merge → profile filter (user-home path) → admission gate (user-home path) → sync to disk → CLI-deploy (user-home path) → optional prune → receipt rewrite → exit. |
...
| `ToolAdapter` | The protocol abstracting everything the engine needs to know about a tool: its `name`, source dir, dest dir, detection, scoped namespaces, project-scoped namespaces, namespace filtering, post-staging transforms. Per-tool adapters live in `tools/`. |
...
- The installer's place among its persistent stores: the read-only source tree, `profiles.toml` and `installer.toml`, the N per-tool destination stores, and the AI tools that consume those stores at their own runtime.
- The installer's internal components: the tool-agnostic `core/` engine (including `orchestrator.py`), the per-tool `tools/` adapters, the per-plugin `plugins/` adapters, and the `cli`/`config` top layer.
- The installer's runtime behaviour: one install invocation's traversal through detect → stage → overlay → merge → profile filter (user-home path) → admission gate (user-home path) → sync → CLI-deploy (user-home path) → (optional) prune → receipt rewrite, plus the collision-merge, per-item sync-decision, and prune sub-flows.
...
| [`c4-l3-engine.md`](c4-l3-engine.md) | **C4 Level 3** — components inside the installer process, grouped by sub-package: the tool-agnostic `core/` engine (`orchestrator`, `model`, `io_port`, `templates`, `staging`, `installignore`, `overlay`, `sync`, `consent`, `run`, `clis`, `receipt*`, `prune_hash`, `prune_flow`, `ownership`, `deploy_gate`/`admission`, `profiles`, `kits`, `merge/*`), the `tools/` adapters, the `plugins/` adapters, and the `cli`/`config` top layer — with the four protocol seams (`ToolAdapter`, `PluginAdapter`, `MergeStrategy`, `IOPort`) as the test-substitution points. `cli.py` is the real top-level driver (staging then sync as two separate whole-fleet passes); `orchestrator.py`, part of `core/`, covers staging only. |
| [`sequences.md`](sequences.md) | **Four sequence diagrams**: (1) end-to-end install — detect → stage → overlay → merge → profile filter (user-home path) → admission gate (user-home path) → sync → CLI-deploy (user-home path) → optional prune → receipt rewrite; (2) collision-merge dispatch — `(FileKind, namespace)` → strategy; (3) sync per-item decision — hash-skip vs diff → confirm → backup → write; (4) prune flow — orphan scan → interactive prune → backup + remove. |
