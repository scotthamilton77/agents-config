...
erDiagram
    ...
    NamedRulesInclude {
        string names  "verbatim comma-list capture; split/trimmed/empty-skipped at flatten time, joined in listed order"
    }
    ...
    Counters {
        ...
        int merged     "collision resolved by a merge strategy (e.g. append_rules, json_union)"
...
Structured config at `packages/installer/installer.toml`, parsed by `core/installer_toml.py`'s `load_installer_toml`. **Designed, parsed, but not yet wired**: nothing in the live install path calls this loader — there is no `Config.tool_overrides` field, and dest resolution goes through `adapter.dest_dir(home)` everywhere (including the prune scan), so a declared override currently has zero runtime effect. The only table is `[tools]` — pruning is **not** configured here; it is driven by the persisted install receipt (below), not a glob list.
...
| Key | Type | Drives (once wired) | Notes |
|---|---|---|---|
| `[tools].<tool>.dest` | str (path) | Intended: tool adapter dest resolution | Parsed into `InstallerToml.tool_dest_overrides`; **not yet consumed** by any caller. Threading it into `adapter.dest_dir(home)` is a deliberate later story. |
...
flowchart LR
    ...
    S1 -->|"staging reads"| M1
    S2 -.->|"parsed; not yet wired into any consumer"| M1
...
| `installer.toml` | **Human** | Permanent | Parsed by `core/installer_toml.py`, but read by nothing in the live install path today — designed, not wired. |
...
| Install receipt (`install-receipt.json` + `.lock`) | **Installer** | Permanent on disk (between runs) | The installer's own persisted state — read at prune start, rewritten at run end (mirrors disk). Trusted state behind an `integrity` digest; held under a single-writer advisory lock. A `--project` install uses a second, project-local instance (`<project-root>/.agents-config/install-receipt.json`) instead of this one. |
