...
over the materialized snapshot, EXTRACTED-only centrality via
...
### PR slice 4 — graphify centrality extractor (EXTRACTED dependency in-degree)

**Goal:** load-bearing axis from graphify's `graph.json`, restricted to **deterministic
(EXTRACTED) dependency edges**. **Test item: 10.**

**Files:**
- Create: `packages/vizsuite/src/vizsuite/extract/centrality.py` — the extractor (§3.6 code).
- Create tests: `packages/vizsuite/tests/unit/test_extract_centrality.py` (fixtures use the real `directed:false` node-link shape with mixed `confidence`, so the `Graph`-vs-`DiGraph` trap and the confidence filter are both exercised).
...
- **Provenance boundary (R3-1) — the load-bearing catch:** in the real graph, **60%
  (4278/7140) of dependency-relation edges are `confidence: INFERRED`** (`uses` is
  100% inferred; `calls` is ~53% inferred). INFERRED edges are agent-produced (Tier 2).
  A "Tier-1 deterministic" centrality axis (spec §6.2) must count **only
  `confidence == "EXTRACTED"` edges** — otherwise Tier-2 inference is laundered into a
  deterministic axis, bypassing the provenance/freshness/schema gate. INFERRED edges
  are out of scope for `.2.1`; if V2 wants them they enter as Tier-2 with citations
  (`.2.3`).
- **Build, don't load:** do NOT `nx.node_link_graph(raw)` — for this `directed:false`
  ...
  file-level `nx.DiGraph` from the EXTRACTED dependency-relation subset, edge
...
**Steps (TDD):**
- [ ] **1. Failing test — confidence filter (item 10 / R3-1):** a fixture with EXTRACTED and INFERRED `calls`/`uses` edges → only EXTRACTED edges contribute to in-degree (an INFERRED-only hub scores zero). Run → FAIL → **2. Green.**
...
- [ ] **5. Failing test — projected via head graph (item 10b):** head-built fixture (`built_at_commit == head_oid`) with a newly-added file's incoming EXTRACTED edges → new hub scores non-zero. Run → FAIL → **6. Green.**
...
- [ ] **11. Gate + commit:** `feat(vizsuite): graphify EXTRACTED-dependency centrality extractor`.
...
are `INFERRED`** (`uses` 100% inferred; `calls` ~53%). The extractor therefore keeps
**only `EXTRACTED` dependency edges** (Tier-1 determinism), builds a file-level
`DiGraph`, drops intra-file edges, and scores in-degree. `nx.node_link_graph` is
deliberately NOT used (it returns an undirected `Graph` with no `in_degree`).
...
def centrality_axis(graph_path: Path, head_oid: str) -> CentralityAxis:
    """Load-bearing axis from graphify. Tier-1 determinism: EXTRACTED edges only
    (INFERRED graph edges are Tier-2, out of scope for .2.1). Intra-file edges dropped;
    projected post-PR centrality via the head-graph preflight (no overlay)."""
    ...
    for link in raw["links"]:
        if link.get("relation") not in DEP_RELATIONS:
            continue
        if link.get("confidence") != "EXTRACTED":                  # Tier-1 determinism (R3-1)
            continue
        src = id_to_file.get(link["source"]); dst = id_to_file.get(link["target"])
        if src is None or dst is None or src == dst:               # drop intra-file
            continue
        g.add_edge(src, dst)                                       # file-level dependency edge
    return CentralityAxis.from_indegree(dict(g.in_degree()))       # normalized 0–1 per file
```
...
| 10 Centrality corrections | 4 | EXTRACTED-only; intra-file edges excluded; head-graph new hub non-zero |
...
- **Spec coverage (`.2.1`):** §4.4 envelope (slices 1/5), §4.6 packaging + script-safe
  ...
  EXTRACTED centrality (slice 4), §6.3 reconciler + snapshot + tracer-bullet framing
...
- **Scope boundaries:** doit + diskcache → `.2.3` (V1 has no funnel/sidecar, §6.3); the
  `viz build` verb is not `.2.1` (§5.6 lists no such verb); INFERRED graphify edges →
  Tier-2 (`.2.3`), excluded from `.2.1`'s deterministic axis; consequence pulled into
  `.2.1` (trivial `.critical-paths` parse — completes the extraction layer).
- **Provenance integrity:** the load-bearing axis counts EXTRACTED graph edges only, so
  no Tier-2 inference is laundered into a "deterministic" axis (R3-1). Per-file checksum
  is one pinned domain (git blob SHA), not an ambiguous hash choice (R3-5).
