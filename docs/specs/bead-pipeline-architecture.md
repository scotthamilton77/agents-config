...
- `[coverage]` — `applicable` (default `true`), `threshold` (default 80), `report-location`, `per-module-pattern`, `format`. Read by green-loop, quality-sweep, and preflight. When `applicable = false`, coverage is opt-out for the project: preflight does NOT fire the human-flag protocol on missing/empty `report-location`, and green-loop / quality-sweep treat coverage as a no-op. Use this for docs-only or config-only repos that have no test infrastructure. When absent or `true`, the existing semantics apply (empty/missing `report-location` triggers human-flag).
