...
The orchestrator dispatches a `tdd-red-team` subagent to write the tests (multi-AC mode on `implement-feature`; single-regression mode on `fix-bug`), then runs the dual-reviewer protocol. The Claude reviewer invokes the `test-review` skill with a checklist covering AC coverage, test-only-method drift, dependency mocking discipline, edge/error paths, white-box minimality, and single-responsibility unit-test structure. The foreign reviewer invokes Codex (`gpt-5.5` default) for adversarial review of added tests in context of surrounding tests, flagging redundant or contradictory tests.
...
**Subagents dispatched.** `tdd-red-team` (opus, high effort) for test writing; `test-review` skill invocation by the orchestrator directly (not a subagent).
...
**Subagents dispatched.** `tdd-green-team` (claude-opus-4-7, effort: high on iteration 1, effort: medium on iterations 2-N per agents-config-7bk.15 TR3) per iteration for implementation work; `quality-reviewer` (claude-opus-4-7, standard 200K context per-iteration runs; claude-opus-4-7[1m] for the final review pass on the full bead diff) per iteration; pure-Claude fresh-eyes subagent on iter 3+. Per-iteration `simplify` skill invocation is dropped in favor of running `simplify` once at the final review pass.
...
| `agents-config-7bk.13` | `resolve-human-bead` skill + `/resolve-human-bead` slash command | Skill and slash command referenced throughout this document and `beads-labels.md`, but not yet present in the plugin tree. Implementation-ready (`brainstormed` + `implementation-ready` + `ralf:required`); awaits run-queue pickup. |
...
| tdd-red-team | claude-opus-4-7 | high | Read, Edit, Write, Grep, Glob, Bash | superpowers:test-driven-development, writing-unit-tests, superpowers:using-git-worktrees | red | Author failing tests in the red-tests stage (multi-AC mode on `implement-feature`; single-regression mode on `fix-bug`). Emits a [`tdd-red-report-v1`](./tdd-red-report-v1.md) YAML to the caller-provided absolute report path. Bead-agnostic. | delivered |
| tdd-green-team | claude-opus-4-7 | medium (high for iter 1) | Read, Edit, Write, Grep, Glob, Bash | superpowers:test-driven-development, superpowers:verification-before-completion, superpowers:using-git-worktrees | green | Implement minimum code to turn failing tests green in the green-loop stage. Emits a [`tdd-green-report-v1`](./tdd-green-report-v1.md) YAML per iteration to the caller-provided absolute report path. Bead-agnostic. | delivered |
| bug-diagnoser | claude-opus-4-7 | high | Read, Grep, Glob, Bash | superpowers:systematic-debugging, superpowers:root-cause-tracing | yellow | Reproduce + root-cause-trace the diagnose stage of `fix-bug`. Emits a [`bug-diagnoser-report-v1`](./bug-diagnoser-report-v1.md) YAML with a non-empty `root_cause_note` to the caller-provided absolute report path. Bead-agnostic. | delivered |
| docs-edits-team | claude-opus-4-7 | high | Read, Edit, Write, Grep, Glob, Bash | (none) | cyan | Apply documented prose/spec edits in the `docs-only` formula's `apply-edits` stage. Emits a [`docs-edits-report-v1`](./docs-edits-report-v1.md) YAML to the caller-provided absolute report path. Bead-agnostic. | delivered |
| pr-comment-fixer-team | claude-opus-4-7 | high | Read, Edit, Write, Grep, Glob, Bash | (none) | orange | Fix a single PR review comment (classify FIX/SKIP/ESCALATE; action COMMITTED_FIX/ALREADY_ADDRESSED/NO_ACTION). Dispatched per-comment by `wait-for-pr-comments`. Emits a [`pr-comment-fix-report-v1`](./pr-comment-fix-report-v1.md) YAML to the caller-provided absolute report path. Bead-agnostic — no `bd` calls and no `.beads` references. | delivered |
...
| Worker agent definitions (`tdd-red-team`, `tdd-green-team`, `bug-diagnoser`, `docs-edits-team`, `pr-comment-fixer-team`) (section 6) | agents-config-acmh.13 (Tier-2 cleanups: deprecation of legacy mega-agent) |
