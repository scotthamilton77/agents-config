...
```
  ...
  - **`roots`** (top-level) is the **allowlist of legitimate installer-owned install roots** — every root the installer has actually written to, maintained across runs as `prior_roots ∪ this-run's live install roots` (the closed tool dest roots plus the route roots of plugins active this run). It is an **independent** record, never derived from `entries`, so a corrupt entry cannot smuggle in a new root. Path validation requires each entry's `root` to be a member (see *Path trust boundary*); this is what proves a *retired* plugin's recorded root was once legitimate while rejecting a hand-edited `root` like `.ssh` that was never an install surface.
...
- **Root legitimacy (allowlist)** — `root` must be a member of the **legitimate-roots allowlist** = the live tool dest roots (closed, always derivable) ∪ the live discovered-plugin route roots ∪ the persisted top-level `roots`. Confining a path under `home` is **not** a sufficient boundary — `~/.ssh` and `~/Documents` are under `home` too — so a `root` that is not a proven install surface is rejected even if `path` is structurally clean. For a **live-known** owner this is also a consistency check (a tool's `root` must equal its `dest_dir(home)`; a discovered plugin's `root` must be one of its live route roots). For a **retired plugin** (no longer discoverable), the persisted `roots` allowlist is what proves the recorded `root` was once a legitimate route root — so its entries prune, while a hand-edited entry whose `root` was never an install surface is rejected as corrupt. This is what lets a retired plugin's route entries become orphans **without** trusting an arbitrary recorded root.
...
- **Malformed receipt path.** An entry whose `path` is absolute, contains `..`, resolves outside its recorded `root`, or whose `root` is not in the persisted `roots` allowlist (so a forged `root` like `.ssh` is rejected even though it is under `$HOME`) is rejected by the path trust boundary → skipped, never an orphan. A bad receipt prunes *less*, never deletes outside the install surface.
...
- **Path trust boundary** — entries with absolute paths, `..` components, or a `path` outside their recorded `root` are skipped; a tool entry whose `root` mismatches the live tool dest is rejected; **a retired plugin's entry whose `root` is in the persisted `roots` allowlist passes validation and IS pruned**, while **a retired/forged entry whose `root` was never an install surface (e.g. `.ssh`, `Documents`) is rejected** even though it is under `home` and structurally clean.
...
- The receipt is a validated deletion-authority input: each entry records its install `root`, and every `root` must be a member of the persisted `roots` allowlist of legitimate install surfaces (live tool/plugin roots ∪ prior roots). A retired plugin's entries validate against the allowlist and prune; an absolute / `..` / outside-`root` path, or a `root` that was never an install surface (`.ssh`, `Documents`), is rejected and never emitted as an orphan. Confinement to `$HOME` is explicitly **not** treated as a sufficient boundary.
