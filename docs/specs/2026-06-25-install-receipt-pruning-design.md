...
```
  ...
  - **`sha256`** is the hex digest of a file's bytes (`sync._sha256` already computes it), `null` for `dir` entries. **v1 uses it as ownership-drift protection when pruning files** (see *Hash-aware file pruning*): a file orphan whose current bytes differ from the recorded digest is treated as a user customization and **relinquished, not deleted**. `dir` entries have no single digest, so they fall back to backup-on-prune; recursive-digest protection for directories is deferred (`fkewj`).
...
### Hash-aware file pruning (ownership-drift protection)

The receipt auto-deletes far more paths than the old hand-listed glob set, so a recorded file the user has since **customized** must not be silently removed when its source is dropped. Before a **file** orphan is sent to `run_prune`, its current on-disk bytes are hashed and compared to the recorded `sha256`:

- **Match** (still our pristine bytes) → prune normally (backup + delete).
- **Mismatch** (user-modified since we installed it) → **relinquish, do not delete**: drop it from the new receipt (added to `relinquished_this_run`, exactly like a consent-declined overwrite) and leave the user's file in place. We installed it, the user made it theirs; we hand ownership back rather than delete their work.
- **Already absent** → no-op; drop from the receipt.

**Directories** (`sha256: null`) have no single digest in v1, so a directory orphan still goes through backup-and-delete — the existing backup makes it recoverable. Recursive-digest protection for directories (refuse auto-delete when contents drifted) is the remaining scope of `fkewj`. This asymmetry is deliberate: file-level protection is nearly free (the digest is already recorded) and covers the common customization case (an edited skill rule, a tweaked script); the directory case needs new machinery and is deferred.
...
- **The receipt mirrors disk.** The new receipt is **not** a blind overwrite of staged items, nor an owner-scoped carve-out. It is the single invariant *an entry leaves the receipt only when it actually leaves the disk or its ownership is relinquished, and enters only when actually written to disk*:
  ...
  `relinquished_this_run` is previously-recorded paths whose ownership we hand back to the user this run — the **consent-declined overwrites of already-recorded entries** (see below) **and the hash-mismatched file orphans** (a recorded file the user customized, identified at prune time — see *Hash-aware file pruning*).
...
| hash-aware **prune filter** | partition file orphans by on-disk `sha256` vs recorded → (prune \| relinquish) | File orphans only; dir orphans pass straight through to `run_prune` |
...
- **Hash-aware file pruning** — a file orphan whose on-disk bytes **match** the recorded `sha256` is pruned (backup + delete); a file orphan the user **modified** (digest mismatch) is **relinquished** (dropped from the receipt, file left in place), even under `--prune --yes`; a `dir` orphan (no digest) still goes through backup-and-delete.
...
- **Directory-level ownership-drift protection** (`agents-config-fkewj`, blocked by this). v1 ships hash-aware protection for **file** orphans (relinquish on digest mismatch — see *Hash-aware file pruning*), but **directory** orphans have no single digest and still go through backup-and-delete. The follow-up records a recursive digest (or per-file manifest) for `dir` entries and refuses auto-delete when the directory's contents have drifted from the recorded owned state, extending the file-level guarantee to skill/agent directories. (Scope narrowed from the original "all sha256-aware prune" now that the file case is in v1.)
...
- v1 prunes **files** hash-aware: a file orphan matching its recorded `sha256` is deleted; a user-modified one (mismatch) is relinquished, not deleted, even under `--prune --yes`. `dir` orphans fall back to backup-and-delete; recursive-digest protection for directories is deferred to `fkewj`.
