...
- **In scope — accidental corruption.** Truncated/partial writes, parse failures, version drift, and stray garbage values are caught and **fail closed** (corrupt receipt → prune disabled, file untouched; see *Receipt lifecycle*). The per-entry validation below additionally bounds garbage paths so a damaged-but-parseable receipt prunes *less*, never wild.
...
- **Out of scope — deliberate forgery.** A hand-crafted, internally-consistent receipt that impersonates a *retired* plugin is trusted (its files prune). Closing this would require either a tamper-proof out-of-band registry (impossible on a single-user machine) or abandoning retired-plugin cleanup; it is accepted as equivalent to direct home write access.

### Path trust boundary

Every entry is validated before it can become an `Orphan`, reusing the installer's existing relpath guard:

- **Structural** — `path` and `root` must both be **relative** (reject absolute) and contain **no `..`**. After rejoining `home`, a relative no-`..` path always resolves under `home`.
- **Containment** — `path` must resolve under `home/root`.
- **Root legitimacy** — resolved by owner kind, *live code first*:
  ...
  - **Retired plugin owner** (not a live tool, not discovered) → `root` must be in the persisted `roots` allowlist. This is **trusted state** (per *Trust model*): it rejects *accidental* garbage roots, but a self-consistent forge that adds a root to both `roots` and an entry is, by the stated threat model, out of scope.
- An entry failing any check is treated as **receipt corruption for that entry**: skipped, logged, **never emitted as an `Orphan`**.

Net effect: for tools and discovered plugins the deletion surface is pinned by live code regardless of the receipt; for retired plugins it is pinned by trusted persisted state. A *damaged* receipt prunes less, never wild; a *forged* one is bounded to impersonating a retired plugin — accepted, because that already requires the write access needed to delete files outright.
...
- The receipt is a validated deletion-authority input under an explicit **trust model** (receipt = trusted installer state, like a package manager's file DB): tool and discovered-plugin roots are taken from **live code** (a forged/corrupt `root` for them is overridden), retired-plugin roots are validated against the persisted `roots` allowlist (trusted state, rejecting accidental garbage), and structural checks (relative, no `..`, containment) bound every path. Accidental corruption fails closed and prunes less, never wild; deliberate forgery of a retired plugin is documented as out of scope (equivalent to direct home write access).
