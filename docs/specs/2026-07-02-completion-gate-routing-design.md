...
23. For every tool adapter's declared namespace tuple plus the shared/plugin/installer roots (§5), assert **effective coverage, not file existence**: seed each required root's `.critical-paths` with a `**` pattern by default (narrower only when a root deliberately excludes a known-safe subpath), then assert a representative file under every declared root actually produces a `critical_path_hits` entry when run through `critical_hits`. An empty or mis-anchored marker file fails this test even though the file exists. Fails if a namespace is added to any adapter without a corresponding, actually-matching marker.
...
**Cap reached.** This was the fifth and final scheduled round; round 5 still reported 2 high findings (both fixed above, unverified by a subsequent round). The pattern across all 5 rounds trended toward narrower, more structural fixes each time (namespace-list gaps → generic derivation; ad hoc dedupe → evidence-preserving contract) rather than repeating the same class of finding, which is the basis for treating this as ready for implementation-plan handoff despite not reaching a clean report. A sixth round was not run.
