...
Ships as a saved named workflow — a bare `.js` script whose body opens with `export const meta = {...}` (source: `src/user/.claude/workflows/quality-gate.js`; `meta.name` = `"quality-gate"`), invoked as `Workflow({name: "quality-gate", args: {...triage JSON...}})`. Codifies the wgclw.14 operational lore:
...
**Loop convergence.** The finder→refute→fix rounds above do **not** use "loop-until-dry" termination — field evidence falsified it (this spec's own 8-round campaign never reached a clean report because the reviewer emits best-N per round rather than draining a pool; wgclw.14's refuter panels showed the mirror failure, zero refutations across all votes). The `quality-gate` workflow **binds to a shared convergence discipline**, it does not define its own. That discipline (authoritative source: `docs/specs/2026-07-03-adversarial-loop-convergence-decision.md`) is a separate design effort with its own bead lineage; the load-bearing decisions it hands this workflow are: a **dual-signal stop** — an *acceptance* exit (ledger clean at the severity floor AND delta discovery dry in the same round, confirmed by a full-fresh certification pass) kept strictly separate from a *termination* exit (round cap / stall / budget) that emits a residual-risk report and never a quality claim; **delta-scoped rounds plus a final certification pass** (a shrinking review surface makes convergence geometric — the structural fix for best-N-forever); and an **evidence-based judge layer** (per-finding validity by citation/repro, a single rank-anchoring triage bench for severity/scope/dedup) that retires the opinion-poll refuter panels. Until that discipline is built, the workflow ships a **capped-round loop that emits the dual-signal residual-risk report** — an honest interim, not a "clean" claim — and adopts the shared predicate when it lands.
...
`scripts/install.sh` is **not** a touch point — it is a thin `exec uv run` stub with no namespace logic (that moved into the Python installer). `merge/registry.py` is **not** a touch point either: workflow files are bare `.js` scripts, so `classify_file` tags them `FileKind.OTHER` (only a `.md` file under a namespace becomes `NAMESPACED_MD`), and the registry's existing `(OTHER, None) → LastWinsSilent` binding already resolves them — the un-wired-namespace `UnknownMergeKeyError` can arise only for a `.md` namespace, which `workflows` is not. `overlay.py::_TOOL_NAMESPACES` (plugin-overlay staging) is out of scope: it matters only if a plugin ships workflows, and none do.

Deliverables:
- Add `workflows` to the three code lists above (`claude.py`, `ownership.py`, `backup.py`); update the one test that pins `PRUNE_NAMESPACES` exactly.
...
**Installer touch-point correction (2026-07-03, implementation-planning recon):** writing the implementation plan required reading the actual installer code, which found the Round-7 "four hardcoded lists including `install.sh`'s two loops" claim **stale**. `scripts/install.sh` is a 6-line `exec uv run` stub with zero namespace logic — the bash loops were removed in the Python migration; codex's "verified by inspection" had read stale test-docstring line numbers. §7.1 corrected accordingly. Noted as a data point for the convergence effort: an adversarial finding asserted a "verified" fact that was false; real ground-truth reconnaissance before acting is what caught it.

**Workflow format + registry-touch-point correction (2026-07-03, implementation recon):** the saved-workflow on-disk format — left open in §7 and flagged for verification in the plan — is a **bare `.js` script** (`export const meta = {...}` first), confirmed against shipped plugin workflows (`…/code-modernization/workflows/*.js`) and persisted session scripts, *not* a `.md` file. This retires the `merge/registry.py` touch-point the entry above had just added: a `.js` workflow classifies as `FileKind.OTHER` (verified in `staging.py::classify_file`), and `resolve` normalizes every non-namespaced kind's namespace to `None` (verified in `registry.py::_normalize`), so `(OTHER, "workflows")` resolves to the already-registered `LastWinsSilent` and can never raise `UnknownMergeKeyError`. That error is reachable only for a `.md` namespace. The real touch-point set is **three** lists — `claude.py`, `ownership.py`, `backup.py` — not four; the dropped registration would have keyed `(NAMESPACED_MD, "workflows")`, an unreachable state. Second data point for the convergence effort: the *same* finding that corrected one stale "verified" claim itself rode an unverified assumption (`.md`), caught only by reading the code that *decides* classification and resolution — recon must bottom out at the deciding code, not one level above it.
