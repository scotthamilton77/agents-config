...
| `src/user/.claude/skills/orchestrating-subagents/SKILL.md` | prose "verify git/gh/bd first" | RTF (light) | verify-state noun becomes `work show <the worker's claimed item id — known to the orchestrator from its own dispatch brief>`; when the id is unknown, fall back to `work list --status in_progress` to select it first (raw `work show` with no id exits `E_USAGE`, so an id or a selection step is mandatory) | C |
