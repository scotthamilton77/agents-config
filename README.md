...
- Syncs `agents/`, `skills/`, `commands/`, `rules/`, and `hooks/` directories using hash comparison per item, and a recursive digest to detect drift inside owned directories
...
- **Scope:** the managed namespaces (`commands` / `skills` / `agents` / `rules` / `hooks` under each tool's config dir, plus any bespoke routes an active plugin declares outside the tool trees). Top-level `*.md` and `settings.json` are never pruned.
