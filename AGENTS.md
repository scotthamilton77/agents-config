...
2. Implement on a worktree branch; never commit to the default branch. Slice small: a PR is one incremental step, and a slice that balloons mid-implementation stops and asks rather than pushing through — large PRs turn cross-model review into a marathon of one-more-thing rounds.
