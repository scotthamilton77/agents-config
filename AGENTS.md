...
- **Exception — the packages under `packages/` are real Python code with mandatory quality gates.** `make ci` (the whole-repo gate CI enforces) runs `ci-installer` + `ci-prgroom` + `ci-workcli` + `ci-vizsuite` + `ci-grind` + `lint-actions` + `spec-lint` + `content-lint` + `content-tests`. Before pushing a change under a gated package run its own gate: `make ci-installer`, `ci-prgroom`, `ci-workcli`, `ci-vizsuite`, or `ci-grind`. Each of those runs lint, format-check, typecheck, coverage, audit, and entry-verify. See the package's own `AGENTS.md` for its scoped workflow. (`packages/pdlc/` and `packages/holding-place/` are early and not yet wired into `make ci`.)
- **Changes under `src/` are gated too, by the two repo-root targets.** `make content-lint` stages the real `src/` tree for every tool and plugin and runs the deploy-time admission gate over it, so an over-cap skill body or a malformed record fails the build here instead of failing someone's install; it also prints the always-on and per-skill token numbers on a pass, so drift is visible before the cliff. `make content-tests` runs every test suite shipped under `src/` and requires every shipped script to have a paired `<stem>_test.*` or `test_<stem>.*` sibling. Neither invokes the installer.
