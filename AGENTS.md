...
- `src/` — **the deployed surface**: everything installed into user space is authored here — shared content under `src/user/.agents/`, per-tool content under `src/user/.claude/` (and `.codex/`, `.gemini/`, `.opencode/`), plugin content under `src/plugins/`. See `src/AGENTS.md` for the layout and the two gates that read the tree.
...
- `.beads/` — the tracker's storage layer; never touched directly (see the tracker-facade rule in Harness Rework above, and `.beads/AGENTS.md`).
