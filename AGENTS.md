...
- `packages/` — standalone uv projects; **not** part of the installed config surface.
  ...
  - `vizsuite/`, `contracts/` — earlier-stage packages. `pdlc/` and `holding-place/` were retired to the archive repository on 2026-08-05; the executor work that would have drawn on their design carries a pointer to them.
