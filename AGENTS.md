...
- `packages/` — standalone uv projects; **not** part of the installed config surface.
  ...
  - `gitclean/` — surveys this repository's worktrees and branches, classifies deletion risk, and deletes only what it can prove is safe. Merge evidence resolves in tiers rather than trusting `git branch --merged`, which under squash merges is wrong in both directions. **Deliberately not on PATH.** Its risk model infers reachability from proxies — a branch's identity from its name, a worktree's danger from whether it is dirty — and review found confirmed data-loss paths that survive a bare sweep. It stays in-tree and inside `make ci` while that model is reworked; until then nothing installs it and no skill drives it.
