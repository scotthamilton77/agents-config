...
- `packages/` — standalone uv projects; **not** part of the installed config surface.
  ...
  - `gitclean/` — surveys this repository's worktrees and branches, classifies deletion risk, and deletes only what it can prove is safe. Merge evidence resolves in tiers rather than trusting `git branch --merged`, which under squash merges is wrong in both directions; the thin `clean-up-git` skill sits over it and owns only the restraint.
  - `pdlc/`, `holding-place/`, `vizsuite/`, `contracts/` — earlier-stage packages
  - `workcli`, `prgroom`, `grind` and `gitclean` are the packages installed onto PATH (`uv tool install`, receipt-tracked, pruned on retirement), landing as the `work`, `prgroom`, `grind` and `gitclean` commands; `CLI_PACKAGES` in `packages/installer/src/installer/core/clis.py` holds that list. Most packages carry their own `AGENTS.md` with a scoped workflow — read it before changing that package.
