...
- `packages/` — standalone uv projects; **not** part of the installed config surface.
  ...
  - `grillui/` — the grilling-session backend: it will serve the session UI, fold the decision log into the context images, and drive the grilling tiers, per `docs/specs/2026-08-18-grilling-ui-v1.md`. Under construction — the append-only session log, typed receipts, the board endpoints, deterministic context images with persistence, and recorded dispatch contexts exist; the status lane, agent drive and the UI serving are still to come.
