...
- Content under `src/` is documentation and templates with no build step — changes there follow existing formatting conventions per file type. It is still mechanically gated: `make ci` measures the real `src/` tree against the admission bar and its token caps, and runs every test suite `src/` ships. Neither gate invokes the installer.
