...
- `packages/` — standalone uv projects; **not** part of the installed config surface.
  ...
  - `workcli`, `prgroom` and `grind` are the packages installed onto PATH (`uv tool install`, receipt-tracked, pruned on retirement), landing as the `work`, `prgroom` and `grind` commands; `CLI_PACKAGES` in `packages/installer/src/installer/core/clis.py` holds that list. Being gated by `make ci` does not put a package on PATH — `gitclean` is gated and withheld. Most packages carry their own `AGENTS.md` with a scoped workflow — read it before changing that package.
