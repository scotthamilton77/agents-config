...
4. Open a PR, then run the review yourself: invoke the `review-panel` skill against the PR and address its findings to quiescence. The panel is a Claude skill, so a session on another tool hands the round to a Claude Code session rather than skipping it. Bot reviews on this repository are off, so nobody else starts a round, and an empty comment thread is a round nobody has run yet. Quiescence is the panel's terminal-clean verdict as `review-verdict` defines it. A change under `packages/prgroom` also runs `make mutants-prgroom` before each round; that gate is advisory, and its surviving mutants are evidence the round reads. A change to that package's `src/` or its `pyproject.toml` takes one of two shapes, and `make version-guard` fails the build on anything else: it bumps the version to a bare release, or it keeps the released version and appends `+partial` to say the tweaks are not worth a reinstall. A bump and the label are exclusive, since a bumped `+partial` version is one the installer refuses to deploy and the review round demands be installed. A bump obligates the human to reinstall the CLI, because a review round posts its verdict through the copy on PATH rather than the one in the tree. Every item gets a disposition in your own inventory; only items that change the code get a reply on the PR. Bookkeeping and meta comments are dispositioned silently — a thread of "no action required" replies is noise the next reader has to wade through.
