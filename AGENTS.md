...
1. Enter the work in the tracker via `work` verbs, as a child of `agents-config-9k9`, with an admission record **and a track**. Milestones are track-exempt, so a child of the milestone inherits nothing and `work create` fails closed with `E_TRACK_REQUIRED` unless you pass `--track NAME`. That failure names every configured track, so the vocabulary is one wrong create away rather than something to look up; the charters deciding which one an item takes are in §3 of `docs/specs/2026-07-15-workcli-track-partition-design.md`. Nesting under a tracked parent instead inherits its track. Installer work also carries the `install` label; nest it under an install epic beneath the owning milestone.
2. Implement on a worktree branch; never commit to the default branch.
3. Verify mechanically before claiming anything. For `packages/**`, and for any skill under `src/` that ships its own tests, that is `make ci` — read the `ci` target in the `Makefile` for its current membership, and note that a single package's gate is not the whole-repo gate. Run it from the root of the tree you are working in: the `Makefile` `cd`s relative to the invoking directory, so a gate run from the main checkout while you are on a worktree branch reports green on code you did not change. Run the gate standalone and read its exit status — never pipe it into a `grep && commit` chain, where the pipeline's status is the grep's and a red gate ships. For prose-only changes, run `doc-lint` — the one gate that reads prose outside `docs/specs/` (and the charter inside it), checking that every backticked citation still resolves — and state what else you checked by hand. `make install-git-hooks` installs a pre-commit check, once per checkout and shared by all its worktrees, that refuses a commit touching a package under `packages/` unless that package's gate recorded exit 0 over the exact content being committed. The check reads only the gates the `Makefile` stamps, so a check you run outside them still rests on you; `--no-verify` bypasses it and is a bypass a human owns. A tree that predates the check prints one line and commits anyway, so an older branch and a bisect are never blocked by it.
4. Open a PR, then run the review yourself: invoke the `review-panel` skill against the PR and address its findings to quiescence. The panel is a Claude skill, so a session on another tool hands the round to a Claude Code session rather than skipping it. Bot reviews on this repository are off, so nobody else starts a round, and an empty comment thread is a round nobody has run yet. Quiescence is the panel's terminal-clean verdict as `review-verdict` defines it. A change under `packages/prgroom` also runs `make mutants-prgroom` before each round; that gate is advisory, and its surviving mutants are evidence the round reads. A change to that package's `src/` or its `pyproject.toml` either bumps the release or leaves it and appends `+partial`, never both, as `packages/prgroom/AGENTS.md` states and `make version-guard` enforces, and a bump obligates the human to reinstall the CLI before the next review round. Every item gets a disposition in your own inventory; only items that change the code get a reply on the PR. Bookkeeping and meta comments are dispositioned silently — a thread of "no action required" replies is noise the next reader has to wade through.
5. **Merge only on an explicit human instruction.** The shared hard-lines state: "Creating a PR is not authorization to merge. Absent an explicit instruction, or a merge policy stated in writing in the project's own configuration, do not merge — branch protection, a green pipeline and an approving review are not one." `project-config.toml`'s live `[merge-policy]` block is that writing, and it states `merge-authorization = "explicit"` — no rule-based policy is configured, and no implementation of one is deployed, so the written policy and this hard line agree: explicit human instruction only. The repository ruleset also requires an approving review (`agents-config-9k9.23`); the configured GitHub App can submit one through `prgroom approve`, and that moves nothing on this line — the App attests the head SHA of a merge a human already instructed, and never supplies the instruction. Read the commented-out `merge-authorization = "rule-based"` line as future work (`agents-config-9k9.64`), not as permission. When the instruction covers a stack of dependent PRs, retarget every one of them to `main` first, then merge one command at a time and confirm each PR's content actually reached `main`: GitHub retargets the next PR's base asynchronously after a squash-merge, so a chained `gh pr merge` can squash into the just-deleted base and still show MERGED.
...
- **Always edit source, never deployed artifacts** — files under `~/.claude/`, `~/.codex/`, `~/.gemini/` and `~/.config/opencode/` are deploy outputs and are overwritten on the next installer run. If you catch yourself editing a path outside `src/`, stop and find the source equivalent. Live-testing a changed skill means invoking the worktree's copy by full path — a bare `Skill()` call and `~/.claude/skills/...` both resolve the deployed copy and silently test other bytes.
...
One line per entry; the detail lives in each directory's own `AGENTS.md`, loaded when you work there.
...
- `.beads/` — the tracker's storage layer; never touched directly, except its own `AGENTS.md`, which this repo owns (see the tracker-facade rule in Harness Rework above).
