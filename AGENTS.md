...
- `packages/` — standalone uv projects; **not** part of the installed config surface.
  ...
  - `workcli`, `prgroom`, `grind` and `executor` are the packages installed onto PATH (`uv tool install`, receipt-tracked, pruned on retirement), landing as the `work`, `prgroom`, `grind` and `executor` commands; `CLI_PACKAGES` in `packages/installer/src/installer/core/clis.py` holds that list. Most packages carry their own `AGENTS.md` with a scoped workflow — read it before changing that package.
