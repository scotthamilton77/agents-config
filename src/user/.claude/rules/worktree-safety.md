...
Claude Code creates and enters worktrees under `.claude/worktrees/` via `EnterWorktree` — Claude's worktree home. Other agents use `<repo-root>/.worktrees/`; know both so agents can collaborate on a shared workspace.
...
- Post-merge cleanup: after a squash-merge, both `git branch -d` and `ExitWorktree` read the branch as unmerged — use `git branch -D` and pass `ExitWorktree` `discard_changes: true`, once the work is confirmed on main.
