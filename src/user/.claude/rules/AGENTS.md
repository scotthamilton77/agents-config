...
Installation is gated on admission: a rule without a complete `admission:` record (`prevents`, `cost`, `remove_when`) in its front matter is dropped at deploy and pruned. This folder is empty today; the record-less rules moved to `archive/src/user/.claude/rules/`.
