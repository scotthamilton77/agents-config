...
| `no-spec` | `--spec` was given without a document, or the attacked document is one the emitter would not have read: bytes that are not UTF-8 text, or nothing but whitespace. Each is refused before a prompt goes out, so no round in hand attacked one, and closing a round over an empty stub clears work to start against criteria nobody wrote. |
| `spec-contains-marker` | The attacked document carries an untrusted-content marker of its own — the emitter's row above says why it refuses rather than rewrite what it hashed. No round in hand attacked this text. |
