...
| `code-reviewer` agent | Security issues in test fixtures, code quality, missing edge cases | Dispatch as subagent on the test files |
| `code-simplifier` agent | Overly complex test setup, redundant assertions, unclear test structure | Dispatch as subagent on the test files after other issues are fixed |

**Workflow:** Apply `testing-anti-patterns` + `writing-unit-tests` criteria first (test-specific), then dispatch `code-reviewer` agent (general quality), then `code-simplifier` agent (cleanup).
...
Organize findings by severity, following the `code-reviewer` agent convention:
