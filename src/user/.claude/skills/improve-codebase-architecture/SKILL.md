...
Placement: Claude tree, because an invocation writes an HTML file and opens it in the user's viewer before the user is asked anything. Not every tool honours a user-invoked declaration; where it goes unhonoured, that report lands on the user's screen unrequested. The procedure is otherwise portable, which is why it looks shareable and is not.
-->
