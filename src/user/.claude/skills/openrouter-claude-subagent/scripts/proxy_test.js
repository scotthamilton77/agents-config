...
// ─── stripDeferredTools ───────────────────────────────────────────
//
// Claude Code declares a `defer_loading` placeholder tool and announces the
// deferred set in a mid-conversation `system` message. OpenRouter answers
// HTTP 400 for any non-Anthropic model ("Deferred custom tools are only
// supported on Anthropic models"), so the whole run dies before it starts.

/** A request body in the shape the harness actually sends. */
function deferredBody(model) {
  return {
    model,
    tools: [
      { name: "Read", input_schema: {} },
      { name: "ToolSearch", input_schema: {} },
      { name: "DeferredToolPlaceholder", defer_loading: true, input_schema: {} },
    ],
    messages: [
      { role: "user", content: [{ type: "text", text: "do the thing" }] },
      { role: "system", content: [{ type: "text", text: "The following deferred tools are now available via ToolSearch." }] },
    ],
  };
}

test("drops the defer_loading placeholder tool for a non-Anthropic model", () => {
  const body = stripDeferredTools(deferredBody("google/gemini-3.1-flash-lite"));

  assert.deepEqual(body.tools.map((t) => t.name), ["Read", "ToolSearch"], "the placeholder is what OpenRouter rejects");
  assert.ok(!body.tools.some((t) => "defer_loading" in t), "no surviving tool may carry the rejected flag");
});

test("demotes the mid-conversation system turn to user and corrects it", () => {
  const body = stripDeferredTools(deferredBody("moonshotai/kimi-k2.7-code"));

  const roles = body.messages.map((m) => m.role);
  assert.deepEqual(roles, ["user", "user"], "the system role mid-conversation is an Anthropic extension");

  const demoted = body.messages[1].content;
  assert.equal(demoted.length, 2, "the original announcement text must be preserved, not discarded");
  assert.match(demoted[0].text, /deferred tools/, "original block kept verbatim");
...
test("leaves a request untouched when no tool defers loading", () => {
  ...
  assert.equal(out.messages[0].role, "system", "an unrelated system turn is not this repair's business");
});

test("tolerates a body with no tools and a string-content system turn", () => {
  assert.doesNotThrow(() => stripDeferredTools({ model: "z-ai/glm-5.2", messages: [] }));

  const body = stripDeferredTools({
    model: "z-ai/glm-5.2",
    tools: [{ name: "X", defer_loading: true }],
    messages: [{ role: "system", content: "plain string" }],
  });
  assert.deepEqual(body.messages[0].content.map((b) => b.text)[0], "plain string", "string content is normalized, not dropped");
});

test("only Anthropic-vendor model ids keep the deferred-tool protocol", () => {
  assert.equal(isAnthropicModel("anthropic/claude-opus-4.8"), true);
