...
/** Only Anthropic's own models accept the deferred-tool protocol below.
 *  Model IDs reach OpenRouter as `vendor/model-slug`. */
function isAnthropicModel(model) {
  return typeof model === "string" && model.startsWith("anthropic/");
}

/** Claude Code can advertise "deferred" tools: tools it deliberately leaves
 *  out of `tools[]` and expects the model to reach through ToolSearch. It
 *  announces them with a placeholder tool carrying `defer_loading: true` and a
 *  mid-conversation `system` message naming what is now reachable. OpenRouter
 *  rejects the placeholder outright for every non-Anthropic model — "Deferred
 *  custom tools are only supported on Anthropic models" — with an HTTP 400
 *  that kills the whole request, so nothing runs at all.
 *
 *  Dropping the placeholder confines the model to the tools actually declared,
 *  which is the only contract those providers can honour. The announcement is
 *  kept but demoted to a user turn — the `system` role mid-conversation is
 *  itself an Anthropic extension — with a correction appended, so the model
 *  is not left hunting for tools it can no longer reach. */
const NO_DEFERRED_TOOLS_NOTE =
  "Correction: no further tools can be loaded during this conversation. " +
  "Only the tools already declared for this request are callable.";

function stripDeferredTools(body) {
  if (!Array.isArray(body.tools)) return body;
  if (!body.tools.some((t) => t?.defer_loading === true)) return body;

  body.tools = body.tools.filter((t) => t?.defer_loading !== true);
  for (const tool of body.tools) delete tool.defer_loading;
