...
| `complete` | `n/a`        | no blocking/critical        | YES                    | converge; fresh-eyes alone decides (no gate to consult). NOTE: workers do NOT emit `gate_status` — per `worker-report-v1.md` §1.1, the orchestrator derives `gate=n/a` ONLY when the entire `evidence` object is `{}` (e.g., synthetic crash report or docs-only / diagnose-only dispatch with no runnable gates). Absence of an `evidence.tests` block alone does NOT yield `gate=n/a`: the derived gate rolls up from whichever blocks ARE present (per §1.1, `pass` when all present blocks have `exit_code: 0` and `skipped: false`, `partial` when any present block is skipped, `fail` when any present block has a non-zero exit code). For a tdd-green-team dispatch in a test-capable repo, omitting the `evidence.tests` block while supplying other green blocks would derive `gate=pass`, not `gate=n/a`; supplying a malformed `tests` block (present but missing required sub-fields) triggers the R1.4.1 synthesis path below. |
...
- A present `evidence` block that is **missing one of its required sub-fields** (per `worker-report-v1.md` §1.1) is treated as a malformed report. Required sub-fields differ by block type: the `evidence.tests` block requires `command`, `exit_code`, `skipped`, `passing`, and `failing`; the `evidence.build`, `evidence.lint`, and `evidence.typecheck` blocks require only `command`, `exit_code`, and `skipped` (the test-count fields `passing` / `failing` do not apply). The orchestrator synthesizes a `status: failed` report with `escalations[].reason: "Worker emitted malformed report"`, wrapping the missing-sub-field name and the raw bytes of the worker's file in `escalations[].detail`.

This extension means: a worker that emits a top-level-parseable YAML but supplies a `tests` block with `command` only (no `exit_code`, no `skipped`) is **not** a convergence candidate — its report fails the synthesis-path check before the derived gate is computed. The same holds for a `build` / `lint` / `typecheck` block that supplies `command` only (no `exit_code`, no `skipped`) — but note that `passing` / `failing` are NOT required for those non-`tests` blocks, so their absence alone is not a malformed-report trigger. Synthesized `status: failed` reports always have `evidence: {}` (per `worker-report-v1.md` §4), so their derived gate is `n/a` — but `status=failed` short-circuits the convergence predicate (the `status=failed` row of the table above) regardless of gate.
...
**N/A case** (when the worker's report carries no `evidence.tests` block at all — e.g., docs-only / config repos where no test runner was exercised — or when the derived gate is `n/a` because the entire `evidence` object is `{}`): there is no `evidence.tests.command` to re-run, so accept `simplify`'s output without re-running tests. There is no measurable trigger; fresh-eyes alone is the convergence signal.
