...
6. **Open it** — unless the user asked you not to, open the finished file in the
   default viewer with the platform opener: `open` (macOS), `xdg-open` (Linux), or
   `start ""` (Windows). Suppress on any "don't open / no auto-open / just save"
   instruction. Either way, print the final path so the reader can find it.
