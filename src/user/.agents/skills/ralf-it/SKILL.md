...
2. **`simplify` skill** — Simplify and refine for clarity
3. **Build quality checks** — `pnpm build && pnpm typecheck && pnpm lint` (or project equivalent)
4. **Tests** — Run the relevant test suite

If the quality-reviewer or `simplify` skill finds significant issues, have them fix what they can. Record what was found for the fresh-eyes subagent.
...
2. Run the **`simplify` skill** one final time
...
**Quality gate:**
- **quality-reviewer** agent — Code review between iterations
- **`simplify`** skill — Code simplification between iterations
