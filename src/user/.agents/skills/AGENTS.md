...
**The pinned commit is the reference copy.** This repository does not vendor full upstream trees, because `Upstream` plus `Source` already identifies the exact bytes and a clone reproduces them on demand. To inspect drift, clone the upstream repository, check out the pinned SHA, and diff that against the deployed copy. When a skill is resynced to a newer upstream commit, bump the SHA and the `Last sync` date in its header in the same change.

**The pin covers body bytes only, and vendoring is per file.** Front matter — `description`, the admission record, `argument-hint` — is house-authored on every artifact, pinned or not, so a drift diff showing front-matter divergence is the convention working rather than drift. A vendored file the host skill has no use for is simply not carried, and the header says which files are vendored. Where a vendored body disagrees with its host skill or with the house rules, the host and the house rules win, and the host skill says so in one sentence rather than the body being edited.

That makes each row's accuracy load-bearing: no local copy stands behind it. A pin that names a commit predating the skill it claims to source is not a stale convenience — it is the only record, and it is wrong. Verify a SHA by hashing the file you are pinning and finding the commit whose blob matches, not by picking a commit near the right date.

**Absence is recorded here, not on the artifact.** A file the host skill declines to carry, and a section cut from a body that is carried, are editorial decisions about this repository: they change no deployed byte, and there is nothing at runtime for them to inform. The provenance header states what the deployed copy *is*; what it is not belongs in the table below, where someone diffing against upstream will look and a loading agent will not.

| Artifact | Not carried, or cut | Why |
|---|---|---|
| `writing-skills` | `references/persuasion-principles.md` (superpowers) | Prescribes stacked imperative boosters, authority appeals and urgency framing, chosen by skill type. The skill's own register split and its `references/anti-patterns.md` both hold that hard MUSTs make a technique document rigid where explanation makes it capable, and the house prompt dialect rules out ALL-CAPS pressure — so the file taught the opposite of its host on the one point it existed to settle. Its empirical claim came from a jailbreak-refusal study on a smaller, older model, which does not transfer to instruction-following; and it named one host's tools in a skill that ships to four. |
| `writing-skills` | `examples/CLAUDE_MD_TESTING.md` (superpowers) | Every scenario premised on the agent finding skills by shelling out over `~/.claude/skills/`, and on a CLAUDE.md that nags it into doing so — the mechanism the skill's own body exists to correct. It was also an unrun plan whose results section held predictions. Editing the paths does not save the framing. |
| `writing-skills` | `references/testing-skills-with-subagents.md` — two dated authoring logs and a duplicate cycle table | The logs are the narrative-example anti-pattern that skill's `references/anti-patterns.md` names in its own first row, at the same date. The table reproduced the file's earlier mapping table in a new column layout, making it the fifth statement of one cycle inside one file. The end recap after it is kept. |
