...
**Testing (HIGH)**: unit tests for business logic, integration tests for critical workflows, edge case + error scenario coverage. Coverage floor on changed code: 80% line / 70% branch (project AGENTS.md may override). Floor is a behavioral minimum, not a quality bar — do not pressure anti-pattern tests to clear it.
