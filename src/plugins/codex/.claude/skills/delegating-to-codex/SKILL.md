...
That agent ships with a separate plugin, not with this one: the plugin is named
`codex`, and it is published by the `openai-codex` marketplace — install the
former from the latter. If the agent does not appear as an agent type in the
session, check whether the `codex` plugin is installed, and if it is not, stop
and tell the user rather than falling back to the raw `codex` binary, which
