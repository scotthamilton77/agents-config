...
- `basic/` — formulas + scripts only (route parity).
- `overlay_only/` — non-colliding overlay files (a command + a rule) with no
  routes, so clean-overlay parity is pinned independently of the route gap.
