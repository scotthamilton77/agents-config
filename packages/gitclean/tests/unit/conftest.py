...
def make_pr(*, number: int = 1, state: str = "OPEN", updated_at: str | None = None) -> PullRequest:
    return PullRequest(
        number=number,
        state=state,
...
def make_branch(
    ...
    *,
    ...
    return Branch(
        ...
        remote=remote,
        head="0" * 40,
        last_activity=last_activity if last_activity is not None else iso(1),
        upstream=upstream,
...
def make_worktree(
    ...
    *,
    ...
    return Worktree(
        ...
        locked=locked,
        prunable=prunable,
        dirty=(dirty_file_count + untracked_file_count) > 0,
