...
def _duration_from_toml(table: dict[str, Any], key: str, default: timedelta) -> timedelta:
    raw = table.get(key)
    if raw is None:
        return default
    if not isinstance(raw, str):
        msg = f"{key} must be a duration string, got {raw!r}"
        raise ValueError(msg)  # noqa: TRY004  # config-domain validation error; ValueError is the loader's uniform type
...
class PrgroomConfig:
    ...
    def load(
        ...
        *,
        ...
        """Resolve config with CLI > env > TOML > default precedence (§3.5)."""
        table = _read_toml(repo_config)

        max_rounds = cls._resolve_max_rounds(table, max_rounds_flag)
        return cls(
            max_rounds=max_rounds,
            review_start_timeout=_duration_from_toml(
                table, "review_start_timeout", DEFAULT_REVIEW_START_TIMEOUT
            ),
            review_finish_timeout=_duration_from_toml(
                table, "review_finish_timeout", DEFAULT_REVIEW_FINISH_TIMEOUT
            ),
            idle_threshold=_duration_from_toml(table, "idle_threshold", DEFAULT_IDLE_THRESHOLD),
        )
