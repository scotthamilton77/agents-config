"""Pure state predicates the §3.3 run-loop and resolver (§3.2) depend on.
...
``has_queued_fix_commits`` is deliberately absent: the state schema carries no
field for the commit queue (the remote tip is the source of truth), so that
signal is an effectful git read the run-loop supplies to the resolver as a bool.
The pure spine never computes it.
"""
...
# Reviewer statuses that a post-push ``rereview`` re-requests (§3.3). After
...
def has_required_reviewers_to_refresh(state: PRGroomingState) -> bool:
    """True iff ≥1 ``required`` reviewer is in ``{not_requested, declined}`` (§3.3).
...
def push_uploaded_commits_this_cycle(
    ...
    """True iff ``_push`` advanced ``last_pushed_head_sha`` during this cycle (§3.2).
...
def push_awaiting_rereview(state: PRGroomingState) -> bool:
    """True iff a review-invalidating HEAD has not yet been rereviewed (§3.3).
...
def new_lifecycle_gate_this_cycle(state: PRGroomingState, *, previous_error: str | None) -> bool:
    """True iff a lifecycle gate APPEARED this cycle — unset->set (§4.6).

    A "new gate" is the transition from no gating error to one (``previous_error is
    None`` and ``state.last_error`` now set) — set this cycle, not set the prior
    cycle. The run-loop uses this to clear
...
def flip_stale_required_reviews(reviewers: dict[str, ReviewerState]) -> bool:
    """Invalidate prior reviews bound to a superseded SHA — shared between the push
    and poll call sites.
