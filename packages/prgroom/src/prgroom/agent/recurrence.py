"""The §7.2 ``recurrence`` signal — a per-item derived input to the fix agent.

prgroom **detects, it does not interpret** (§7.2): for an item carrying a prior
...
snapshot is ``lifecycle/snapshot.py``'s job; this module owns only the data
shape and its wire serialization (the field set and the "omit empty
``prior_commits``" rule §7.2 specifies inline).
"""
...
class Recurrence:
    """One item's recurrence signal (§7.2). Frozen + slots: a hashable value type."""
    ...
    def to_dict(self) -> dict[str, Any]:
        """Serialize for the fix snapshot. ``prior_commits`` omitted when empty (§7.2)."""
        d: dict[str, Any] = {
