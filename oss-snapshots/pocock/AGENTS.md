...
| `tdd/` | `engineering/tdd` | Compare-and-improve | **amalgamate → `writing-unit-tests`** (pending) | Pull into native `writing-unit-tests`: anti-horizontal-slicing rule + diagram, proactive interface-design pre-phase. Do not modify `superpowers:test-driven-development` (plugin-sourced, not ours). |
