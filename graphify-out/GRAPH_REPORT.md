...
4. `PRRef` - 178 edges
5. `SidecarStore` - 172 edges
6. `BdResult` - 159 edges
7. `StagingPlan` - 139 edges
8. `RecordedRunner` - 135 edges
9. `ScriptedTrackerRunner` - 126 edges
10. `BdBackend` - 116 edges
...
- `pr_ref()` --calls--> `PRRef`  [INFERRED]
  packages/prgroom/tests/conftest.py → packages/prgroom/src/prgroom/prsession/pr_ref.py
- `store()` --calls--> `InMemoryStore`  [INFERRED]
  packages/prgroom/tests/conftest.py → packages/prgroom/src/prgroom/prsession/memory.py
...
Cohesion: 0.06
Nodes (10): Bm(), draw(), fu(), iu(), jm(), lu(), qa(), qu (+2 more)
...
Cohesion: 0.18
Nodes (14): estate(), _in_estate(), Estate scope extractor — the canonical file set (plan §3.5.2, spec §6.2).  The o, The canonical file set at an immutable revision → `{path: blob_sha}`.      Blob, LsTreeRow, One `git ls-tree -r` row: ``<mode> <obj_type> <blob_sha>\\t<path>``.      A Name, NamedTuple, _blob() (+6 more)
...
Cohesion: 0.4
Nodes (6): _attempts_path(), budget_exhausted(), bump_attempts(), True once >= max_attempts no-go's have been recorded for this PR+base., Increment (and persist) the no-go counter for this PR+base; return new value., _read_attempts()

### Community 332 - "Community 332"
Cohesion: 0.53
Nodes (5): main(), _parse_commit(), Reject path-traversal / separator chars in a filename component (fail closed)., _safe_component(), _state_dir()
...
Cohesion: 0.33
Nodes (6): bead-verifier agent (haiku, mechanical evidence), already_addressed SHA-discovery procedure, Completion Gate (review/simplify/verify), fix_outcome (committed/already_addressed/failed), Per-comment subagent contract, verify-checklist skill

### Community 335 - "Community 335"
Cohesion: 0.33
Nodes (6): gpt-5.3-codex-spark model, gpt-5.4 model, gpt-5.4-mini model, codex-routing rule, codex:gpt-5-4-prompting skill, codex-companion.mjs
